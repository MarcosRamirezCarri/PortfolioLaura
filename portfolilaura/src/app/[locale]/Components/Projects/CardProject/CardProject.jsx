  ...
  useEffect(() => {
    ...
    return (
      <div className="w-full flex flex-col relative h-[70vh] ">
        <h1 data-aos='fade-right' className="text-lg lg:text-xl select-none flex flex-row gap-2 text-left self-start text-offSalmon-100 font-semibold font-spartan ml-4 lg:ml-16">{Especialidad}</h1>
        <h1 data-aos='fade-left' className={` lg:text-3xl select-none flex flex-row ${name === 'Kayak WAVE FISHING' ? 'text-lg' : 'text-xl'  } gap-2 text-center self-start text-offSalmon-200 font-semibold font-spartan ml-4 lg:ml-16 mb-5`}>{name} |<p className="text-salmon-200">{Empresa}</p></h1>
        <div className="flex flex-col lg:grid gap-2 w-[90vw] lg:grid-cols-3 self-center ">
<div data-aos='fade' className="bg-offSalmon-300  lg:w-fit lg:h-fit p-3 rounded flex flex-col">
<Image width={400} onClick={() =>openModal(image)} height={400} alt="ProjectImg" src={image} className="h-60 w-52 lg:h-96 select-none lg:w-64 place-self-center cursor-pointer"/>
</div>
<div className="transparent rounded flex  col-span-2 flex-col gap-2 items-baseline ">
<p data-aos='fade-left'  className={` lg:text-2xl font-spartan ${name === 'Comunicacion B2B' ? 'text-md' : 'text-lg'  } text-offSalmon-100 text-justify`}>&apos;{texto1}&apos;</p>
